...
# -----------------------------
PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))  # Now app.py is in project root
ALLOWED_ROOTS = {
...
def api_files_list():
    ...
    for dirpath, dirnames, filenames in os.walk(base):
        for d in dirnames:
            result.append({
                "type": "dir",
                "path": os.path.relpath(os.path.join(dirpath, d), PROJECT_ROOT)
            })
        for f in filenames:
            result.append({
                "type": "file",
                "path": os.path.relpath(os.path.join(dirpath, f), PROJECT_ROOT)
            })
...
if __name__ == "__main__":
    # By default serve on 5173 to avoid clashing with common dev ports
